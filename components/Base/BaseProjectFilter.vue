<script setup lang="ts">
	...
	import { ProjectStatus } from "~/helper/data";
	...
	const selectedStatusValue = ref<any>({
		label: "",
		value: "",
	});

	onMounted(async () => {
		filterData.status = route.query.projectStatus ? route.query.projectStatus : "";
		filterData.signingEntity = route.query.signingEntity ? route.query.signingEntity : "";

		selectedStatusValue.value = {
			label: route.query.projectStatus ?? "",
			value: route.query.projectStatus ? parseInt(route.query.projectStatus) : "",
		};
	});
	...
	const getfilterData = computed(() => {
		if (filterData.status || filterData.signingEntity) {
			return true;
		} else {
			return false;
		}
	...
	const getSelectedStatus = computed(() => {
		return ProjectStatus.find((status: any) => status.value == filterData.status);
	});
