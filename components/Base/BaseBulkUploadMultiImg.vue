...
<script setup lang="ts">
	...
	// 				if (field.mime_type === "application/pdf") {
	// 					fileList.value.push({
	...
	const onChange: UploadProps["onChange"] = (uploadFile: any, uploadFiles) => {
		...
		if (fileSizeInMB < 10 && uploadFile.raw.size > 0) {
			if (
	...
	const handleExceed: UploadProps["onExceed"] = (files) => {
		showError.value = false;
		if (props.limit < props.limit) {
			upload.value!.clearFiles();
			const file = files[0] as UploadRawFile;
			file.uid = genFileId();
			upload.value!.handleStart(file);
		} else {
			errorMessage.value.push(t('formValidations.max_100_files'));
			showError.value = true;
		}
	};
