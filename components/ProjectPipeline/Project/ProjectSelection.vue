<script setup lang="ts">
	...
	const tableData = ref([
		{
			columnName: "tenant_suppliers.full_name",
			columnLabel: t("formLabels.supplier_name"),
		},
		{
			columnName: "adjudication_score",
			columnLabel: t("formLabels.adjudication_score"),
		},
		{
			columnName: "due_diligence",
			columnLabel: t("formLabels.due_diligence"),
		},
		{
			columnName: "site_visit",
			columnLabel: t("formLabels.site_visit"),
		},
		{
			columnName: "further_commentary",
			columnLabel: t("formLabels.further_commentary"),
		},
		{
			columnName: "priority_order",
			columnLabel: t("formLabels.priority_order"),
		},
	]);
	...
	const changeStatus = async (data: any, value: any) => {
		...
		const statusData = { ...data, status: value, stage: "selection" };
		try {
...
<template>
	<div class="p-4">
		<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
			...
			<div>
				<label class="font-bold block">{{ $t("formLabels.start_date_of_selection") }}</label>
				<p class="break-words">{{ editData.start_date ? moment(`${editData.start_date}`).format("DD/MM/YYYY") : "-" }}</p>
			</div>

			<div>
				<label class="font-bold block">{{ $t("formLabels.target_completion_date_of_selection") }}</label>
				<p class="break-words">
			...
			<div>
				<label class="font-bold block">{{ $t("formLabels.final_specification") }}</label>
				<p class="break-words">{{ editData.final_specification || "-" }}</p>
			</div>
			...
			<div class="col-span-1 sm:col-span-2 md:col-span-3 xl:col-span-4 gap-4">
				...
				<el-table :data="editData?.stakeholder_details" :row-class-name="tableRowClassName" stripe>
					<el-table-column v-for="column in tableColumns" :key="column.id" :prop="column.columnName" :label="column.columnLabel" />
				</el-table>
			</div>

			<div class="col-span-1 sm:col-span-2 md:col-span-3 xl:col-span-4 gap-4">
				<label class="font-bold block text-lg">{{ $t("formLabels.supplier_details") }}</label>
				<el-table :data="editData?.supplier_details" :row-class-name="tableRowClassName" stripe>
					<el-table-column v-for="column in tableData" :key="column.id" :prop="column.columnName" :label="column.columnLabel" />
				</el-table>
			</div>
		...
		<div class="text-right">
			...
			<novok-button
				v-if="!isChangeStatus && editData.active_tab !== 'selection'"
				class="my-1"
				type="info"
				:loading="loading"
				:disabled="loading"
