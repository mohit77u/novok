<script setup lang="ts">
	...
	const tableColumns = ref([
		...
		{
			columnName: "approval_provided",
			columnLabel: t("formLabels.approval_provided"),
		},
	]);
	...
	const changeStatus = async (data: any, value: any) => {
		...
		const statusData = { ...data, status: value, stage: "signing" };
		try {
...
<template>
	<div class="p-4">
		<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
			...
			<div>
				<label class="font-bold block">{{ $t("formLabels.start_date_of_signing") }}</label>
				<p class="break-words">{{ editData.start_date ? moment(`${editData.start_date}`).format("DD/MM/YYYY") : "-" }}</p>
			</div>

			<div>
				<label class="font-bold block">{{ $t("formLabels.target_completion_date_of_signing") }}</label>
				<p class="break-words">
			...
			<div>
				<label class="font-bold block">{{ $t("formLabels.suppliers_ability_to_deliver_requirements") }}</label>
				<p class="break-words">{{ editData.suppliers_ability_to_deliver_requirements || "-" }}</p>
			</div>

			<div>
				<label class="font-bold block">{{ $t("formLabels.selection_rationale") }}</label>
				<p class="break-words">{{ editData.selection_rationale || "-" }}</p>
			</div>

			<div>
				<label class="font-bold block">{{ $t("formLabels.preferred_supplier_due_diligence") }}</label>
				<p class="break-words">{{ editData.preferred_supplier_due_diligence || "-" }}</p>
			</div>

			<div>
				<label class="font-bold block">{{ $t("formLabels.implementation_transition_plans") }}</label>
				<p class="break-words">{{ editData.implementation || "-" }}</p>
			</div>
			...
			<div>
				...
				<p class="break-words">{{ editData.contract?.contract_number || "-" }}</p>
			</div>
		...
		<div class="text-right">
			...
			<novok-button
				v-if="!isChangeStatus && editData.active_tab !== 'signing'"
				class="my-1"
				type="info"
				:loading="loading"
				:disabled="loading"
