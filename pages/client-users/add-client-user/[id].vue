<script setup lang="ts">
	...
	const isAttentionForCharge = ref(false);
	...
	const submitAddData = async () => {
		try {
			...
			const res: any = await useApiFetch("/client-users", {
				method: "POST",
				body: formData,
			});

			if (res?.data?.is_attention_for_charge) {
				isAttentionForCharge.value = true;
			} else {
				useToast("success", res.message);
				navigateTo(
					localePath({
						path: "/client-users",
						query: { client_admin_id: route.params?.id },
					})
				);
			}
		} catch {
	...
	const okClick = async () => {
		isAttentionForCharge.value = false;
		formData.is_agree_for_payment = 1;
		await submitAddData();
	};
...
<template>
	<el-form v-loading="fullscreenLoading" :class="{ 'overflow-hidden': fullscreenLoading }" class="flex flex-col relative" :model="form">
		<div id="super-admin-form">
			<el-dialog v-model="isAttentionForCharge" class="max-w-[540px] w-[90%] rounded-2xl noHeader" center :align-center="true" @close="cancel">
				<h1 class="text-3xl font-bold mb-5 mt-4 text-dark/100 text-center">{{ $t("header.attention") }}!</h1>
				<div>
					{{ $t("messages.is_attention_for_charge") }}
				</div>
				<template #footer>
			...
			<div class="bg-[#fff] p-6">
				<div class="grid md:grid-cols-2 lg:grid-cols-6 gap-5 lg:gap-6">
					...
					<div class="lg:col-span-3 mb-0">
						...
						<el-radio-group v-model="formData.account_status">
				...
				<div class="pt-6 text-right">
					<novok-button class="btn-dark" @click.prevent="cancelClick"
